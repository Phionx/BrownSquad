...
/**
 * The class is the superclass for text editing widgets. I have copied the
 * source from TextCoponent in jcurses lib to this class. I then made everything
 * public access so that I could access some of the more useful methods for my
 * purposes. Specifically I wanted to simply be able to plot a string at a given
 * location x,y in any foreground and background color.
 * 
 */
public class NewTextComponent extends Widget {

	public int _width = 0;
	public int _height = 0;

	public int _cursPosX = 0;
	public int _cursPosY = 0;

	public int _firstChar = 0;
	public int _firstLine = 0;
	...
	public NewTextComponent(int width, int height, String text) {
		_width = width;
		_height = height;
		setText(text);
		super.setSize(new Rectangle(width, height));
	 ...
	 * 
	 */
	public NewTextComponent(int width, int height) {
		this(width, height, "");
	}
	...
	public NewTextComponent(String text) {
		this(-1, -1, text);
	}
	 ...
	 *            text to set
	 * @param dispatchEvent
	 *            if true, the widget is repainted
	 */
	...
	public void updateText(boolean dispatchEvent) {
	 ...
	 * example text area)
	 */
	public void drawAdditionalThings() {
		// nothing
	}
	 ...
	 */
	public void refreshAdditionalThings() {
		// nothing
	}
	...
	public Rectangle getTextRectangle(int firstLine) {
	...
	public Rectangle getLineRectangle(int firstLine) {
		Rectangle rect = getTextRectangle();
		int y = ((firstLine - _firstLine) >= 0) ? (firstLine - _firstLine) : 0;
		rect.setLocation(rect.getX(), rect.getY() + y);
		rect.setHeight(1);
	...
	public static CharColor __textComponentDefaultColors = new CharColor(
	...
	public static CharColor __focusedTextComponentDefaultColors = new CharColor(
			CharColor.BLUE, CharColor.WHITE, CharColor.REVERSE);
	public CharColor _focusedTextComponentColors = getFocusedTextComponentDefaultColors();
	...
	public static CharColor __cursorDefaultColors = new CharColor(
			CharColor.BLACK, CharColor.WHITE);
	public CharColor _cursorColors = getCursorDefaultColors();
	...
	public void drawChar(int x, int y, CharColor colors, char c) {
		int x1 = x - _firstChar;
		int y1 = y - _firstLine;
		String toPrint = (c == 0) ? " " : replaceTextLineForPrinting("" + c);
		Toolkit.printString(toPrint, getTextRectangle().getX() + x1,
				getTextRectangle().getY() + y1, colors);
	}

	public void printString(String s, int x, int y, CharColor colors) {
		Toolkit.printString(s, x, y, colors);
	}
	...
	public void drawLine(int index) {
		Rectangle rect = getTextRectangle();
		CharColor colors = hasFocus() ? getFocusedTextComponentColors()
				: getColors();
		int firstLine = getFirstLineNumber();
		...
		if ((firstChar < length) && ((index - firstLine) < rect.getHeight())) {
			...
			int length3 = (length2 > rect.getWidth()) ? rect.getWidth()
					: length2;
			Toolkit.printString(replaceTextLineForPrinting(_text.substring(pos
					+ firstChar, pos + firstChar + length3)), rect.getX(),
					rect.getY() + index - firstLine, colors);
		}
	}
	...
	public void drawBox(Rectangle rect) {
		CharColor colors = hasFocus() ? getFocusedTextComponentColors()
				: getColors();
	...
	public char getCharacterAtCursorPosition() {
		char result = 0;
		if (_text.length() > 0) {
	...
	public boolean isTextChanged(int x, int y) {
		return !((_firstChar == x) && (_firstLine == y));
	}

	public boolean isCursorChanged(int x, int y) {
		return !((_cursPosX == x) && (_cursPosY == y));
	}

	public void redrawAfterCursorMove(int bCursorPosX, int bCursorPosY,
			int bFirstChar, int bFirstLine, char bChar) {
	...
	public void redrawOldChar(int bCursorPosX, int bCursorPosY, char bChar) {
		CharColor colors = hasFocus() ? getFocusedTextComponentColors()
				: getColors();
	...
	public void redrawAfterTextChange(int bCursorPosX, int bCursorPosY,
			int bFirstChar, int bFirstLine) {
		if (isTextChanged(bFirstChar, bFirstLine)) {
			paint();
		} else if (isCursorChanged(bCursorPosX, bCursorPosY)) {
	...
	public void redrawLine(int index) {
	 ...
	 * 
	 * @param line
	 *            a text line to replace, contains no line breaks
	 * @return decoded line
	 */

	public String replaceTextLineForPrinting(String line) {
		return line;
	}
	...
	public boolean handleInput(InputChar ch) {
		int inputCode = ch.getCode();
		inputListener.keyCodeReceived(inputCode);
		return true;

		/*
		 * int bCursorPosX = _cursPosX; int bCursorPosY = _cursPosY; int
		 * bFirstChar = _firstChar; int bFirstLine = _firstLine; char bChar =
		 * getCharacterAtCursorPosition();
		 * 
		 * 
		 * if (ch.getCode() == InputChar.KEY_RIGHT) {
		 * setCursorLocation(_cursPosX+1, _cursPosY);
		 * redrawAfterCursorMove(bCursorPosX
		 * ,bCursorPosY,bFirstChar,bFirstLine,bChar); return true; } else if
		 * (ch.getCode() == InputChar.KEY_LEFT) { setCursorLocation(_cursPosX-1,
		 * _cursPosY);
		 * redrawAfterCursorMove(bCursorPosX,bCursorPosY,bFirstChar,bFirstLine
		 * ,bChar); return true; } else if (ch.getCode() == InputChar.KEY_UP) {
		 * setCursorLocation(_cursPosX, _cursPosY-1);
		 * redrawAfterCursorMove(bCursorPosX
		 * ,bCursorPosY,bFirstChar,bFirstLine,bChar); return true; } else if
		 * (ch.getCode() == InputChar.KEY_DOWN) { setCursorLocation(_cursPosX,
		 * _cursPosY+1);
		 * redrawAfterCursorMove(bCursorPosX,bCursorPosY,bFirstChar
		 * ,bFirstLine,bChar); return true; } else if (ch.getCode() ==
		 * InputChar.KEY_HOME) { setCursorLocation(_cursPosX, 0);
		 * redrawAfterCursorMove
		 * (bCursorPosX,bCursorPosY,bFirstChar,bFirstLine,bChar); return true; }
		 * else if (ch.getCode() == InputChar.KEY_END) {
		 * setCursorLocation(_cursPosX, getTextHeight()-1);
		 * redrawAfterCursorMove
		 * (bCursorPosX,bCursorPosY,bFirstChar,bFirstLine,bChar); return true; }
		 * else if (ch.getCode() == InputChar.KEY_NPAGE) { int newYPos = 0; if
		 * (getCurrentPageNumber() < (getPageSize()-1)) { newYPos =
		 * getPaging().getIndexByPageOffset
		 * (getCurrentPageNumber()+1,getCurrentPageOffset()); } else { newYPos =
		 * getTextHeight()-1; } setCursorLocation(_cursPosX, newYPos, true);
		 * redrawAfterCursorMove
		 * (bCursorPosX,bCursorPosY,bFirstChar,bFirstLine,bChar); return true; }
		 * else if (ch.getCode() == InputChar.KEY_PPAGE) { int newYPos = 0; if
		 * (getCurrentPageNumber() > 0) { newYPos =
		 * getPaging().getIndexByPageOffset
		 * (getCurrentPageNumber()-1,getCurrentPageOffset()); } else { newYPos =
		 * 0; } setCursorLocation(_cursPosX, newYPos, true);
		 * redrawAfterCursorMove
		 * (bCursorPosX,bCursorPosY,bFirstChar,bFirstLine,bChar); } else if
		 * (ch.getCode() == InputChar.KEY_BACKSPACE) {
		 * deleteCharBeforeCursorLocation(); if (_cursPosX == 0) { if (_cursPosY
		 * > 0) { int y = _cursPosY-1; if (y<0) { y=0; } int x = ((Integer)
		 * _lineLengths.get(y)).intValue(); setCursorLocation(x,y);
		 * redrawAfterTextChange(bCursorPosX, bCursorPosY, bFirstChar,
		 * bFirstLine); } } else { setCursorLocation(_cursPosX-1, _cursPosY);
		 * redrawLine(_cursPosY); }
		 * 
		 * return true; } else if (!ch.isSpecialCode()) { char c =
		 * ch.getCharacter(); insertCharAtCursorLocation(c); if ( c == '\n') {
		 * setCursorLocation(0, _cursPosY+1); redrawAfterTextChange(bCursorPosX,
		 * bCursorPosY, bFirstChar, bFirstLine); } else {
		 * setCursorLocation(_cursPosX+1, _cursPosY); redrawLine(_cursPosY); }
		 * return true; }
		 * 
		 * return false;
		 */
	 ...
	 * 
	 * @param x
	 *            new x cursor coordinate within the text
	 * @param y
	 *            new y cursor coordinate within the text
	 */
	public void setCursorLocation(int x, int y) {
		setCursorLocation(x, y, false);
	}

	public void setCursorLocation(int x, int y, boolean pageAlignment) {
		if (y < 0) {
			_cursPosY = 0;
	...
	public void insertCharAtCursorLocation(char c) {
		int pos = ((Integer) _lines.get(_cursPosY)).intValue() + _cursPosX;
		_text.insert(pos, c);
		updateText();
	}

	public void deleteCharBeforeCursorLocation() {
		int pos = ((Integer) _lines.get(_cursPosY)).intValue() + _cursPosX;
		if (pos > 0) {
	...
	public ValueChangedListenerManager _listenerManager = new ValueChangedListenerManager();
